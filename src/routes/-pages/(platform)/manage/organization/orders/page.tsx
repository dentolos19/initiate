...
export default function Page() {
  ...
  function setFilter(value: OrderStatus | "all") {
    ...
    router.push(`/manage/organization/orders?${params.toString()}`);
  }
  ...
  const loadOrders = () => {
    ...
    Promise.all([
      backend.organization.getOrganizationOrders("current", filter === "all" ? undefined : filter),
      backend.organization.getOrganizationOrderStatistics("current"),
    ])
      .then(([orders, stats]) => {
        setOrders(orders);
        setStatistics(stats);
  ...
  async function handleContact(userId: string) {
    ...
    await backend.messages
      .createRoom(userId, organization.id)
      .then((room) => {
  ...
  useEffect(loadOrders, [organization, filter]);
    ...
    <div className={"flex h-full flex-col"}>
      ...
      <div className={"border-b shadow-sm"}>
        <div className={"grid grid-cols-1 gap-1 md:grid-cols-2 lg:grid-cols-4"}>
          <div className={"flex items-center justify-between border-r p-6 last:border-r-0"}>
            <div>
              <h2 className={"text-muted-foreground text-sm font-medium"}>Total Orders</h2>
              <p className={"mt-1 text-2xl font-bold"}>{statistics.totalOrders}</p>
            </div>
          ...
          <div className={"flex items-center justify-between border-r p-6 last:border-r-0"}>
            <div>
              ...
              <p className={"mt-1 text-2xl font-bold"}>{statistics.pendingConfirmation}</p>
            </div>
          ...
          <div className={"flex items-center justify-between border-r p-6 last:border-r-0"}>
            <div>
              ...
              <p className={"mt-1 text-2xl font-bold"}>{statistics.pendingCompletion}</p>
            </div>
          ...
          <div className={"flex items-center justify-between p-6"}>
            <div>
              ...
              <p className={"mt-1 text-2xl font-bold"}>{statistics.completedOrders}</p>
            </div>
          ...
          <TableBody>
              ...
              <TableRow>
                <TableCell colSpan={7} className={"text-muted-foreground py-12 text-center"}>
                  <div className={"flex flex-col items-center gap-2"}>
                    ...
                    <p className={"text-sm"}>No orders have been placed for your services yet.</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
            ...
            {orders.map((order) => (
              <TableRow key={order.id} className={"hover:bg-muted/50"}>
                ...
                <TableCell>
                  <Link className={"hover:underline"} href={`/users/${order.user?.id}`}>
                ...
                <TableCell>
                  <div className={"flex items-center gap-2"}>
                    <Button variant={"default"} size={"sm"} asChild>
                      <Link href={`/manage/organization/orders/${order.id}`}>
                        <ShoppingBagIcon className={"h-4 w-4"} />
                        <span>View Details</span>
                      </Link>
                    </Button>
                    <Button variant={"outline"} size={"sm"} onClick={() => handleContact(order.userId!)}>
